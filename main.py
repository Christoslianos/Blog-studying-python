...
class User(UserMixin, db.Model):
    ...
    ##the author refers to the author property in blogpost
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You have already register your email, try to log in")
        ...
        db.session.commit()
        # authenticate the user with flash login
        login_user(new_user)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            return redirect(url_for('login'))
        # check the passowrd has against the password hash
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login for leaving a comment")
            return redirect(url_for("login"))
        new_comment = Comment(
